/* arch/arm/mach-omap2/board-kona-vibrator.c
 *
...
static int kona_create_secvib_sysfs(void)
{
...
static int vibrator_enable(int actr_idx, int on)
{
	...
	/* actr_idx will not be used for kona becase we have only one */
	if (unlikely(gptimer == NULL))
		return -EINVAL;

	if (!motor_ldo) {
		motor_ldo = regulator_get(NULL, "V_MOT_3.0V");
		if (IS_ERR(motor_ldo)) {
...
static int pwm_init(void)
{
	...
	omap_dm_timer_set_pwm(gptimer, 1, 1,
...
void __init omap4_kona_vibrator_init(void)
{
	platform_device_register(&vibrator_device);
	kona_create_secvib_sysfs();
