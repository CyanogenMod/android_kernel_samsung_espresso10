/* arch/arm/mach-omap2/board-superior-vibrator.c
 *
 * Copyright (C) 2012 Samsung Electronics Co, Ltd
 *
 * Based on mach-omap2/board-palau-vibrator.c
 *
...
static int superior_create_secvib_sysfs(void)
{
	int ret;
...
static int vibrator_enable(int actr_idx, int on)
{
	...
	/* actr_idx will not be used for superior becase we have only one */
	...
	if (on) {
		if (!regulator_is_enabled(motor_ldo))
...
void __init omap4_superior_vibrator_init(void)
{
	platform_device_register(&vibrator_device);
	superior_create_secvib_sysfs();
