...
#ifdef CONFIG_SPI_LOOPBACK_TEST
...
#ifndef CONFIG_FEATURE_SAMSUNG_SPI
static
#endif
...
{
	u32 tmp_cmd;
	u32 start_cmd = 0x45674567;

	if (!od) {
		pr_err("[%s]ipcspi: Dev is NULL, but try to access\n",
			__func__);
		return -EFAULT;
	}
...
{
	if (!od) {
		pr_err("[%s]onedram: Dev is NULL, but try to access\n",
			__func__);
		return -EFAULT;
	}
...
static irqreturn_t ipc_spi_irq_handler(int irq, void *data)
{
...
static int ipc_spi_vm_pagefault
...
static u32 ipc_spi_get_send_vbuff_command(void)
{
	...
	memcpy((void *)&cmd, (void *)(p_virtual_buff + \
		ONEDRAM_REG_OFFSET + 64), sizeof(cmd));

	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) get mailbox_BA cmd : 0x%x\n", __LINE__, cmd); */
...
static void ipc_spi_set_send_vbuff_command_clear(void)
{
	...
	memcpy((void *)(p_virtual_buff + \
		ONEDRAM_REG_OFFSET + 64), (void *)&cmd, sizeof(cmd));
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) clear mailbox_BA cmd : 0x%x\n", __LINE__, cmd); */
}
...
static inline void ipc_spi_update_tail_of_vbuff_format_tx(u32 u_tail)
{
	...
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update FMT tx_tail : %d\n", __LINE__, u_tail); */
}

static inline void ipc_spi_update_head_of_vbuff_format_rx(u32 u_head)
{
	memcpy((void *)(p_virtual_buff + 20 + 4),
		(void *)&u_head, sizeof(u_head));
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update FMT rx_head : %d\n", __LINE__, u_head); */
}
...
static inline void ipc_spi_update_tail_of_vbuff_raw_tx(u32 u_tail)
{
	...
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update RAW tx_tail : %d\n", __LINE__, u_tail); */
}

static inline void ipc_spi_update_head_of_vbuff_raw_rx(u32 u_head)
{
	memcpy((void *)(p_virtual_buff + 36 + 4),
		(void *)&u_head, sizeof(u_head));
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update RAW rx_head : %d\n", __LINE__, u_head); */
}
...
static inline void ipc_spi_update_tail_of_vbuff_rfs_tx(u32 u_tail)
{
	...
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update RFS tx_tail : %d\n", __LINE__, u_tail); */
}

static inline void ipc_spi_update_head_of_vbuff_rfs_rx(u32 u_head)
{
	memcpy((void *)(p_virtual_buff + 52 + 4),
		(void *)&u_head, sizeof(u_head));
	/* dev_dbg(&p_ipc_spi->dev,
	"(%d) update RFS rx_head : %d\n", __LINE__, u_head); */
}
...
static void ipc_spi_copy_from_vbuff_format_tx
	(void *p_des, u32 offset_vbuff, u32 copy_len)
{
	if ((offset_vbuff + copy_len) <= FMT_SZ) {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			FMT_OUT + offset_vbuff), copy_len);
	} else {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			FMT_OUT + offset_vbuff), FMT_SZ - offset_vbuff);
		memcpy((void *)(p_des + (FMT_SZ - offset_vbuff)),
			(void *)(p_virtual_buff + FMT_OUT),
			copy_len - (FMT_SZ - offset_vbuff));
	}
}

static void ipc_spi_copy_from_vbuff_raw_tx
	(void *p_des, u32 offset_vbuff, u32 copy_len)
{
	if ((offset_vbuff + copy_len) <= RAW_SZ) {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			RAW_OUT + offset_vbuff), copy_len);
	} else {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			RAW_OUT + offset_vbuff), RAW_SZ - offset_vbuff);
		memcpy((void *)(p_des + (RAW_SZ - offset_vbuff)),
			(void *)(p_virtual_buff + RAW_OUT),
			copy_len - (RAW_SZ - offset_vbuff));
	}
}

static void ipc_spi_copy_from_vbuff_rfs_tx
	(void *p_des, u32 offset_vbuff, u32 copy_len)
{
	if ((offset_vbuff + copy_len) <= RFS_SZ) {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			RFS_OUT + offset_vbuff), copy_len);
	} else {
		memcpy((void *)p_des, (void *)(p_virtual_buff + \
			RFS_OUT + offset_vbuff), RFS_SZ - offset_vbuff);
		memcpy((void *)(p_des + (RFS_SZ - offset_vbuff)),
			(void *)(p_virtual_buff + RFS_OUT),
			copy_len - (RFS_SZ - offset_vbuff));
	}
}
...
{
	struct spi_protocol_header *tx_header =
		(struct spi_protocol_header *)tx_b;
	u16 mux = 0;
	...
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header)),
		(void *)&mux, sizeof(mux));
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux)), (void *)&bof, sizeof(bof));
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux) + sizeof(bof)), (void *)&pkt_len, sizeof(pkt_len));
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux) + sizeof(bof) + sizeof(pkt_len) + 1),
		(void *)&ipc_len, sizeof(ipc_len));
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux) + sizeof(bof) + sizeof(pkt_len) + 4),
		(void *)&ipc_header, sizeof(ipc_header));
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux) + sizeof(bof) + sizeof(pkt_len) + 4 + 4),
		(void *)test_data, LOOP_SIZE-18);
	memcpy((void *)(tx_b + sizeof(struct spi_protocol_header) + \
		sizeof(mux) + sizeof(bof) + pkt_len),
		(void *)&eof, sizeof(eof));

	tx_header->current_data_size = sizeof(mux) + \
		sizeof(bof) + pkt_len + sizeof(eof);
	tx_header->next_data_size = DEF_BUF_SIZE >> 2;

	pr_debug("[SPI DUMP] LOOP TX :[");
	for (i = 0 ; i < 30 ; i++)
		pr_debug("%02x ", *((u8 *)(tx_b + i)));
	pr_debug("| ");
...
COPY_TO_VBUFF_FMT_RETRY:
	...
	if (head + len <= FMT_SZ) {
		...
		memcpy((void *)(p_virtual_buff + FMT_IN + head),
			data, FMT_SZ - head);
		memcpy((void *)(p_virtual_buff + FMT_IN),
			(void *)(data + (FMT_SZ - head)),
			len - (FMT_SZ - head));
	}
...
COPY_TO_VBUFF_RAW_RETRY:
	...
	if (head + len <= RAW_SZ) {
		...
		memcpy((void *)(p_virtual_buff + RAW_IN + head),
			data, RAW_SZ - head);
		memcpy((void *)(p_virtual_buff + RAW_IN),
			(void *)(data + (RAW_SZ - head)),
			len - (RAW_SZ - head));
	}
...
COPY_TO_VBUFF_RFS_RETRY:
	...
	if (head + len <= RFS_SZ) {
		...
		memcpy((void *)(p_virtual_buff + RFS_IN + head),
			data, RFS_SZ - head);
		memcpy((void *)(p_virtual_buff + RFS_IN),
			(void *)(data + (RFS_SZ - head)),
			len - (RFS_SZ - head));
	}
...
static int ipc_spi_thread(void *data)
{
	...
	u8 rx_loopback_data = 0;
	int i, rx_loopback_count = 0;
#endif

	if (sec_bootmode == 5) {
		pr_info("[LPM mode] ipc_spi_thread exit!\n");
		goto exit;
	}

	daemonize("ipc_spi_thread");

	pr_debug("[%s] ipc_spi_thread start.\n", __func__);
	boot_done = 1;

	wait_for_completion(&ril_init);

	pr_debug("[%s] ril_init completed.\n", __func__);

	if (!p_ipc_spi) {
		pr_err("[%s] p_ipc_spi is NULL.\n", __func__);
	...
	if (!tx_buf) {
		dev_err(&p_ipc_spi->dev, "[%s] tx_buf kmalloc fail.",
			__func__);
	...
	if (!rx_buf) {
		dev_err(&p_ipc_spi->dev, "[%s] rx_buf  kmalloc fail.",
			__func__);
	...
	if (!test_data) {
		dev_err(&p_ipc_spi->dev, "[%s] test_data  kmalloc fail.",
			__func__);
	...
	pr_debug("[IPC_SPI] Start IPC Communication. MRDY : %d, SRDY : %d\n",
	...
	while (1) {
		...
		if (!ipc_spi_check_send_data()) { /* no send data */
			if (down_trylock(&srdy_sem)) { /* no srdy sem */
				dev_dbg(&p_ipc_spi->dev,
					"(%d) no data and no sem, wait tx-srdy event.\n",
					__LINE__);

				transfer_thread_waiting = 1;

				skip_SRDY_chk = 0;
				 /* wait event(tx or srdy) */
		...
		do {
			if (loop_back_test) {
				if (tx_header->current_data_size == 0x3) {
					...
					memset((void *)test_data,
						tx_loopback_data, LOOP_SIZE-18);
					ipc_spi_prepare_loopback_tx_data
						(tx_buf, rx_buf , test_data);
...
RETRY_WAIT_SEM:
				...
				if (down_timeout(&srdy_sem, 2 * HZ)) {
					dev_err(&p_ipc_spi->dev, "(%d) SRDY TimeOUT!!! MRDY : %d, SRDY : %d\n",
						__LINE__,
						gpio_get_value(gpio_mrdy),
						gpio_get_value(gpio_srdy));
			...
			/* host to network */
			ipc_spi_swap_data_htn(tx_buf, DEF_BUF_SIZE + 4);

			retval = ipc_spi_tx_rx_sync(
				tx_buf, rx_buf, DEF_BUF_SIZE + 4);
			if (retval != 0)
...
/*				dev_dbg(&p_ipc_spi->dev,
			...
			/* network to host */
			ipc_spi_swap_data_nth(rx_buf, DEF_BUF_SIZE + 4);
			*(u32 *)tx_header = ntohl(*(u32 *)tx_header);

			pr_debug("[SPI DUMP] LOOP RX :]");
			for (i = 0 ; i < 30 ; i++)
				pr_debug("%02x ", *((u8 *)(rx_buf + i)));
			...
			if ((rx_buf[14] == 0x0A) && (rx_buf[15] == 0x0B) \
				&& (rx_buf[16] == 0x01)) {
				rx_loopback_data =
					rx_buf[17];
				...
				for (i = 0; i < LOOP_SIZE - 18; i++) {
					if (rx_buf[17+i] != rx_loopback_data) {
						pr_err(
							"[Loopback] %dth %02x != %02x Invaild data!\n",
							i,
							rx_loopback_data,
							rx_buf[17+i]);
						goto exit;
				...
				if (i == (LOOP_SIZE - 18))
					pr_err("[Loopback] %02x is OK!\n",
					rx_loopback_data);

				rx_loopback_count++;
				if ((rx_loopback_count % 10) == 0 \
					&& rx_loopback_data == 0x0A) {
					tx_loopback_data = 1;
					loop_back_test = 1;
					pr_err("[Loopback] rx_loopback_count = %d\n",
						rx_loopback_count);
				}
				rx_header->RTSCTS = 0;
				rx_header->more = 0;
			...
			dev_dbg(&p_ipc_spi->dev,
			"(%d) check more, CTS : %d, rx more : %d, tx more : %d\n",
			__LINE__, rx_header->RTSCTS,
			rx_header->more, tx_header->more);
			msleep(2000);
		} while (rx_header->RTSCTS || \
			rx_header->more || tx_header->more);
...
{
	u16	crc = 0;				/* CRC value*/
	u8	*src_ptr = NULL;		   /* source buffer pointer*/
	int	dest_len = 0;		   /* output buffer length*/
	u8	*dest_ptr = NULL;		   /* dest buffer pointer*/
...
static int ipc_spi_send_modem_bin_execute_cmd
	(struct ipc_spi *od, u8 *spi_ptr, u32 spi_size, u16 spi_type,
		u16 spi_crc, struct sprd_image_buf *sprd_img)
{
	int i, retval = 0;
	u16 send_packet_size = 0;
	u8 *send_packet_data = NULL;
	u16 d1_crc = 0;
	u16 d2_crc = spi_crc;
	u16 type = spi_type;
...
/*
	dev_dbg(&p_ipc_spi->dev,
	"[SPI DUMP] TX_D1(%d) : [%02x %02x %02x %02x %02x %02x %02x %02x]\n",
...
/*
	pr_err("[SPI DUMP] TX_D2(%d) : [ ", send_packet_size);
	for (i = 0; i < 20; i++)
		pr_err("%02x ", *((u8 *)(rx_buf + i)));
	pr_err("... ");

	for (i = send_packet_size - 10; i > send_packet_size - 1; i--)
		pr_err("%02x ", *((u8 *)(rx_buf + i)));
	pr_err("]\n");
*/
	...
	if (down_timeout(&srdy_sem, 2 * HZ)) {
		dev_err(&p_ipc_spi->dev,
			"(%d) SRDY TimeOUT!!! SRDY : %d, SEM : %d\n",
			__LINE__, gpio_get_value(gpio_srdy), srdy_sem.count);
		goto err;
	}
...
ACK:
	...
	if (down_timeout(&srdy_sem, 2 * HZ)) {
		...
		dev_err(&p_ipc_spi->dev,
			...
			sprd_img->tx_b[7]);
		dev_err(&p_ipc_spi->dev,
			"[SPI DUMP] TX_D2(%d) : [%02x %02x %02x %02x %02x %02x %02x %02x]\n",
...
/*
	pr_err("[SPI DUMP] TX_D2(%d) : [ ", sprd_img->rx_size);
	for (i = 0; i < 15; i++)
...
static int ipc_spi_send_modem_bin_xmit_img
	(struct ipc_spi *od, enum image_type type, struct image_buf *img)
{
	int retval = 0;
	...
	if (!sprd_img.tx_b) {
		dev_err(&p_ipc_spi->dev, "(%d) tx_b kmalloc fail.",
			__LINE__);
		return -ENOMEM;
	}
	...
	if (!sprd_img.rx_b) {
		dev_err(&p_ipc_spi->dev, "(%d) rx_b kmalloc fail.",
			__LINE__);
		retval = -ENOMEM;
		goto err3;
	}
	...
	if (!sprd_img.encoded_tx_b) {
		dev_err(&p_ipc_spi->dev, "(%d) encoded_tx_b kmalloc fail.",
			__LINE__);
		retval = -ENOMEM;
		goto err2;
	}
	...
	if (!sprd_img.decoded_rx_b) {
		dev_err(&p_ipc_spi->dev, "(%d) encoded_rx_b kmalloc fail.",
			__LINE__);
		retval = -ENOMEM;
		goto err1;
	}
	...
	dev_dbg(&p_ipc_spi->dev,
		"(%d) ipc_spi_send_modem_bin_xmit_img type : %d.\n",
		__LINE__, type);
	...
	case MODEM_MAIN:
		...
		img->buf  =
			(unsigned char *)(p_virtual_buff + 0x30 + fdl1_size);
		img->offset = img->length + fdl1_size + 0x30;
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save MAIN to img.\n",
			__LINE__);
	...
	case MODEM_DSP:
		...
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save DSP to img.\n",
			__LINE__);
	...
	case MODEM_NV:
		...
		img->offset += img->length;
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save NV to img.\n",
			__LINE__);
	...
	case MODEM_EFS:
		...
		img->offset += img->length;
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save EFS to img.\n",
			__LINE__);
	...
	case MODEM_KERNEL:
		...
		img->offset = img->length + fdl1_size + 0x30;
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save KERNEL to img.\n",
			__LINE__);
	...
	case MODEM_USER:
		...
		img->offset += img->length;
		dev_dbg(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img save USER to img.\n",
			__LINE__);
	...
	case MODEM_RUN:
		...
		*(sprd_img.tx_b+2) = 0x00; *(sprd_img.tx_b+3) = 0x00;
		...
		if (retval < 0) {
			dev_err(&p_ipc_spi->dev,
				"encode_msg(Transfer Start) error!retval = %d",
				retval);
			goto err0;
		}
#else
		...
		retval = ipc_spi_send_modem_bin_execute_cmd
			(od, spi_ptr, spi_size, spi_type, crc, &sprd_img);
		if (retval < 0) {
			dev_err(&p_ipc_spi->dev,
				"(%d) ipc_spi_send_modem_bin_execute_cmd fail : %d",
				__LINE__, retval);
			goto err0;
		}
		return retval;
	...
	default:
		dev_err(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img wrong : %d.",
			__LINE__, type);
		goto err0;
	}
	...
	*(sprd_img.tx_b+2) = 0x00; *(sprd_img.tx_b+3) = 0x08;
	...
	if (retval < 0) {
		dev_err(&p_ipc_spi->dev,
			"encode_msg(Transfer Start) error!retval = %d", retval);
		goto err0;
	}
#else
	sprd_img.tx_size = 8;
	...
	for (i = 0; send_size < img->length; i++) {
		...
		*(sprd_img.tx_b+0) = 0x00;
		*(sprd_img.tx_b+1) = 0x02;	/* type */
		M_16_SWAP(sprd_packet_size);
		memcpy((sprd_img.tx_b + 2),
			&sprd_packet_size, sizeof(sprd_packet_size));
		M_16_SWAP(sprd_packet_size);
		...
		if (retval < 0) {
			dev_err(&p_ipc_spi->dev,
				"encode_msg(TransferData_%d) error!retval = %d",
				i, retval);
			goto err0;
		}
#else
		...
		if (retval < 0) {
			dev_err(&p_ipc_spi->dev,
				"(%d) ipc_spi_send_modem_bin_execute_cmd fail : %d, %d",
				__LINE__, retval, i);
			goto err0;
		}
		...
		if (!(i % 100))
			dev_dbg(&p_ipc_spi->dev,
			"(%d) [%d] 0x%x size done, rest size: 0x%x\n",
	...
	*(sprd_img.tx_b+2) = 0x00; *(sprd_img.tx_b+3) = 0x00;
	...
	if (retval < 0) {
		dev_err(&p_ipc_spi->dev,
			"encode_msg(TransferEnd) error!retval = %d", retval);
		goto err0;
	}
#else
	memset(sprd_img.encoded_tx_b, 0, SPRD_BLOCK_SIZE * 2);
...
static void ipc_spi_send_modem_bin(struct work_struct *send_modem_w)
{
	...
	struct ipc_spi_send_modem_bin_workq_data *smw
		= container_of(send_modem_w,
		struct ipc_spi_send_modem_bin_workq_data, send_modem_w);
	struct ipc_spi *od = smw->od;

	tick1 = jiffies_to_msecs(jiffies);

	pr_debug("[SPI DUMP] p_virtual_buff : [ ");
	for (i = 0; i < 20; i++)
		pr_debug("%02x ", *((u8 *)(p_virtual_buff + i)));
	pr_debug("]\n");
	...
	if (retval < 0) {
		dev_err(&p_ipc_spi->dev,
			"(%d) ipc_spi_send_modem_bin_xmit_img fail : %d",
			__LINE__, retval);
		goto err;
	}
#else
	...
	if (retval < 0) {
		dev_err(&p_ipc_spi->dev, "(%d) ipc_spi_send_modem_bin_xmit_img fail : %d",
			__LINE__, retval);
		goto err;
	}
#endif
...
static int __devinit ipc_spi_platform_probe(struct platform_device *pdev)
{
	...
	/* Init work structure */
	ipc_spi_send_modem_work_data =
		kmalloc(sizeof(struct ipc_spi_send_modem_bin_workq_data),
		GFP_ATOMIC);
	...
	INIT_WORK(&ipc_spi_send_modem_work_data->send_modem_w,
		ipc_spi_send_modem_bin);
...
#ifdef CONFIG_PM
static int ipc_spi_platform_suspend
...
static int ipc_spi_platform_resume
...
int ipc_spi_thread_restart()
{
	...
	pr_debug("[IPC_SPI] ipc_spi_thread_restart\n");
	...
	if (retval < 0) {
		pr_err(
			"[%s] platform_driver_register ERROR : %d\n",
			__func__, retval);
...
static int __init ipc_spi_init(void)
{
	...
	pr_info("[%s](%d) init Done - 1011260100.\n",
		__func__, __LINE__);
