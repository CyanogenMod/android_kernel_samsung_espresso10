...
bool sec_hal_fg_reset(struct i2c_client *client)
{
	...
	if (max17050_read_reg(client, MAX17050_REG_MISCCFG, data) < 0)
		return;

	/* Set bit10 makes quick start */
	data[1] |= (0x1 << 2);
	if (max17050_write_reg(client, MAX17050_REG_MISCCFG, data) < 0)
		return false;
...
bool sec_hal_fg_fuelalert_process(void *irq_data, bool is_fuel_alerted)
{
	...
	dev_dbg(&fuelgauge->client->dev,
		"%s: FUEL GAUGE IRQ (%d)\n",
		 __func__,
		 gpio_get_value(irq_to_gpio(fuelgauge->pdata->fg_irq)));
