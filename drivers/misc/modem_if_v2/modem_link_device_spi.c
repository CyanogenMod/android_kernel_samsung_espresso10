...
=====================================*/
void if_spi_set_restart(void)
{
	struct link_device *ld = &p_spild->ld;

	pr_info("[SPI] if_spi_set_restart(spi_main_state=[%d])\n",
...
void if_spi_thread_restart(void)
{
	...
	platform_driver_unregister(&if_spi_platform_driver);

	retval = platform_driver_register(&if_spi_platform_driver);
	if (retval < 0) {
		pr_err("[%s] platform_driver_register ERROR : %d\n",
			__func__, retval);
	}
