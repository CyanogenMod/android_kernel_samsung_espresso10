...
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
	...
	if (instance_data->query.has_sensor_assignment == 1) {
		...
		control->reg_15->length =
				instance_data->query.num_of_rx_electrodes;
		control->reg_15->regs =
	...
	if (instance_data->query.curve_compensation_mode == 1
		...
		control->reg_36->regs = kzalloc(size *
				sizeof(struct f54_ad_control_36n), GFP_KERNEL);
	...
	if (instance_data->query.has_per_frequency_noise_control == 1) {
		...
		control->reg_38->length = instance_data->
					query.number_of_sensing_frequencies;
		...
		control->reg_39->length = instance_data->
					query.number_of_sensing_frequencies;
		control->reg_39->regs =
				kzalloc(control->reg_39->length *
		...
		control->reg_40->length = instance_data->
					query.number_of_sensing_frequencies;
		control->reg_40->regs =
				kzalloc(control->reg_40->length *
