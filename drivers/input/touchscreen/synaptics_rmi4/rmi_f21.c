...
DEVICE_ATTR(rezero, RMI_WO_ATTR, rmi_show_error, rmi_fn_21_rezero_store);
...
static int rmi_f21_initialize(struct rmi_function_container *fc)
{
	...
	retval = rmi_read_block(fc->rmi_dev, instance_data->query.address,
				(u8 *) &instance_data->query,
				sizeof(instance_data->query.regs));
	if (retval < 0) {
		dev_err(&fc->dev,
	...
	instance_data->control.reg_4->length =
	    instance_data->query.max_force_sensor_count;
	...
	instance_data->control.reg_5->length =
	    instance_data->query.max_force_sensor_count;
	...
	instance_data->control.reg_6->length =
	    instance_data->query.max_force_sensor_count;
	...
	instance_data->control.reg_7->length =
	    instance_data->query.max_force_sensor_count;
	...
	instance_data->control.reg_8->length =
	    instance_data->query.max_force_sensor_count;
	...
	instance_data->data.reg_0__1.force_hi_lo =
	    kzalloc(2 * instance_data->query.max_force_sensor_count *
...
static int rmi_f21_create_sysfs(struct rmi_function_container *fc)
{
	...
	switch (int_num) {
	case 7:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_6)) <
		    0) {
			dev_err(&fc->dev,
	...
	case 6:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_5)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
	...
	case 5:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_4)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
	...
	case 4:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_3)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
	...
	case 3:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_2)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
	...
	case 2:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_1)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
	...
	case 1:
		if (sysfs_create_file(&fc->dev.kobj, attrify(int_en_force_0)) <
		    0) {
			dev_err(&fc->dev,
				"Failed to create control sysfs files.");
			return -ENODEV;
...
/* Control */
	...
	show_store_union_struct_unsigned(control, reg_0__3,
					force_click_threshold)
...
/* repeated register functions */
	...
	show_store_repeated_union_struct_unsigned(control, reg_7,
					transmitter_force_sensor)
	show_store_repeated_union_struct_unsigned(control,
					reg_8, receiver_force_sensor)
				    ...
				    struct device_attribute *attr, char *buf)
{
	...
	if (result < 0) {
		dev_err(dev,
			"%s : Could not read regtype at 0x%x\nData may be outdated.",
			__func__, data->data.reg_0__1.address);
	}
	temp = buf;
	for (i = 0; i < reg_length; i++) {
		result = snprintf(temp, PAGE_SIZE - size, "%d ",
				  data->data.reg_0__1.force_hi_lo[i] * (2 << 3)
				  + data->data.reg_0__1.force_hi_lo[i +
								reg_length]);
		if (result < 0) {
