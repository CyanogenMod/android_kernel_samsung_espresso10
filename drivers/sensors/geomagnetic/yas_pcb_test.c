...
static int yas_pcb_is_flow_occued(struct yas_pcb_vector *);
...
static int yas_pcb_test4(int *x, int *y1, int *y2)
{
	...
	if ((NULL == x) || (NULL == y1) || (NULL == y2))
		return YAS_PCB_ERROR_ARG;
...
static int yas_pcb_test6(int *sx, int *sy)
{
	...
	if ((NULL != sx) && (NULL != sy)) {
		...
		if (YAS_PCB_NO_ERROR == result) {
			...
			result = yas_pcb_measure(pN, &temperature,
						u08Command,
						YAS_PCB_INT_NOTCHECK);
			...
			if (YAS_PCB_NO_ERROR == result) {
				...
				result = yas_pcb_is_flow_occued(pP);
				if (YAS_PCB_NO_ERROR == result)
...
{
	int nRet = YAS_PCB_ERROR_ARG;
	struct yas_pcb_vector stOhxy1y2, stOhxyz;

	if ((NULL != ohx) && (NULL != ohy) && (NULL != ohz)) {
